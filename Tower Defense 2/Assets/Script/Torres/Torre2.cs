...
public class Torre2 : MonoBehaviour
{
    ...
    void ataque()
    {
        while (_pontoSpawm < _base._spawmTiro.Length)
        {
            Instantiate(_base._tiro, _base._spawmTiro[_pontoSpawm].position, _base._spawmTiro[_pontoSpawm].rotation);
            _pontoSpawm++;
        }
    ...
    void gPhysics2D()
    {
        ...
        _base._spawnBlockLocal = Physics2D.OverlapCircle(transform.position, 0.55f, _base._blockSpawm);       
    }
